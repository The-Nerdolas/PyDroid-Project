...
def main():
    ...
    configGradle(path,nameProject,packageName)
    execGradle(path,nameProject)
    modifArq(path,nameProject,packageName)
    


def configGradle(path,nameProject,packageName):
    #Configurações
    ...
    buildPackage = open(path + nameProject + "/build.gradle", "r")
    linhas = buildPackage.readlines()
    linhas.insert(29, '        applicationId "'+packageName+'"')
    buildPackage = open(path + nameProject + "/build.gradle", "w")
    buildPackage.writelines(linhas) 
    buildPackage.close()
